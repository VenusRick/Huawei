...
## 2026-09-18（归因与收口）
- 真实成绩分组口径入交付件：握手可见五类 acc 0.874/macro-F1 0.897；
  无握手段（gitlab）如实标不可分
- 流式+资源回收（closed 上限/evict 证据/TLS 缓冲 64MB/generator 全路径交付）
- 按需计算（operators.py + --on-demand）+ §5.3 QUIC 三态 + tgp 薄化
- 94 用例全绿；归因+A 实验（换类反证 CSTNET 类互扰）入档
